    ...
    while True:
        if (curr_idx == len(current_indices)) and (prev_idx == len(previous_indices)):
            ...
            if prev_workload not in current_values[:,0]:
                ...
                slack_msg_sender.send_slack_message("gcp compare_data.py line 31: workload error")
            ...
            if curr_workload not in previous_values[:,0]:
                ...
                slack_msg_sender.send_slack_message("gcp compare_data.py line 42: workload error")
                raise Exception('workload error')
            break
        ...
        if prev_workload != curr_workload:
            if curr_workload not in previous_values[:,0]:
                ...
                slack_msg_sender.send_slack_message("gcp compare_data.py line 60: workload error")
                raise Exception('workload error')
        else:
